...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        Cookies.set('accessToken', responseJson.data.accessToken, {
          expires: 7,
          path: '/',
        });
        if (responseJson.data.refreshToken) {
          Cookies.set('refreshToken', responseJson.data.refreshToken, {
            expires: 7,
            path: '/',
          });
        }
        ...
        setErrorMessage(responseJson.message || 'Login failed.');
      }
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className='min-h-screen bg-gray-50 flex items-center justify-center p-6 relative overflow-hidden'>
      ...
      <div className='absolute top-0 right-0 w-125 h-125 bg-blue-100/50 rounded-full blur-[100px] -mr-64 -mt-64' />
      <div className='absolute bottom-0 left-0 w-125 h-125 bg-purple-100/50 rounded-full blur-[100px] -ml-64 -mb-64' />
      ...
      >
        ...
        <div className='text-center mb-10'>
          <div className='inline-flex items-center justify-center w-16 h-16 bg-digital-blue rounded-3xl shadow-2xl shadow-blue-500/30 mb-6'>
            <span className='text-white font-black text-3xl'>X</span>
          </div>
          <h1 className='text-3xl font-black text-gray-900 tracking-tight'>
            Admin Login
          </h1>
          <p className='text-gray-500 mt-2 font-bold tracking-tight text-sm'>
            Chào mừng bạn đến với hệ thống quản trị Xalo Media
          </p>
        </div>
          ...
          <form className='space-y-6' onSubmit={handleSubmit}>
            ...
            <div className='space-y-2'>
              <div className='flex items-center justify-between ml-1'>
                ...
                <button
                  type='button'
                  className='text-[10px] font-black text-digital-blue uppercase tracking-widest hover:underline'
                >
                  Quên mật khẩu?
                </button>
              </div>
